               ...
               <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 shrink-0">
                    ...
                    <div className="lg:hidden flex">
                         <button
                              ...
                              className="px-4 py-2 btn-primary rounded-xl font-semibold text-sm  shadow-sm flex items-center gap-2"
                         >
...
}.Value -replace 'bg-blue-600\S*', 'btn-primary' -replace 'hover:bg-blue-\d+', '' -replace 'shadow-blue-\d+/\d+', '' w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 flex items-center justify-center shrink-0 ml-4 shadow-sm">
                                                  <User className="w-5 h-5 text-slate-600 dark:text-slate-300" />
                                             </div>
                                        )}
                                   </div>
                         ...
                         <div className="p-4 md:p-6 border-t border-slate-100 dark:border-slate-800 z-10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md">
                              ...
                              <form onSubmit={handleSendMessage} className="flex gap-3 relative max-w-4xl mx-auto">
                                   ...
                                   <button
                                        type="submit"
                                        disabled={isTyping || !inputMessage.trim()}
                                        className="absolute right-2 top-2 bottom-2 aspect-square  import React, { useState, useRef, useEffect } from 'react';
import { Send, Bot, User, Sparkles, MessageSquare, Briefcase, TrendingUp, Plus, Trash2, Edit2 } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
...
}.Value -replace 'bg-blue-600\S*', 'btn-primary' -replace 'hover:bg-blue-\d+', '' -replace 'shadow-blue-\d+/\d+', '' 
                                   >
                                        <Send className="w-5 h-5" />
                                   </button>
                              </form>
